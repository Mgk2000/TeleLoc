...
// Переменная для хранения IP адреса текущего собеседника (защита от Broadcast мусора)
static QHostAddress m_lastPeerIp = QHostAddress::Broadcast;
...
void NetworkEngine::resetRegistration() {
    ...
    emit callEnded(); // Принудительно глушим звук при сбросе профиля
}
...
void NetworkEngine::sendAudioPacket(const QByteArray &audioData) {
    ...
    out << QString("AUDIO_DATA") << m_myName << QString("ALL") << audioData;
...
void NetworkEngine::sendTextMessage(const QString &text) {
    ...
    QHostAddress targetIp = m_lastPeerIp.isNull() ? QHostAddress::Broadcast : m_lastPeerIp;
    m_socket->writeDatagram(datagram, targetIp, m_port);
...
void NetworkEngine::readPendingDatagrams() {
    while (m_socket->hasPendingDatagrams()) {
        ...
        m_lastPeerIp = senderIp;

        if (type == "AUDIO_DATA") {
            QByteArray audioBytes;
            in >> audioBytes;
            emit sendAudioBlock(audioBytes);
        }
            ...
            if (m_activeChatPeer != from) {
                ...
                // ИСПРАВЛЕНО: Смертельный msleep полностью удален, UI поток свободен!
            ...
            m_socket->writeDatagram(datagramAck, senderIp, m_port);
        }
            ...
            while (it != m_offlineQueue.end()) {
                if (it->to == from && it->text == text) {
                    it = m_offlineQueue.erase(it);
                    qDebug() << "===> [ПЕЙДЖЕР] Успешная доставка к" << from;
                } else {
                    ++it;
                }
            }
            ...
            m_socket->writeDatagram(datagramPong, senderIp, m_port);
        }
            ...
            for (const auto &msg : m_offlineQueue) {
                if (msg.to == from) {
                    ...
                    m_socket->writeDatagram(datagramMsg, senderIp, m_port);
                }
            }
        }
    }
